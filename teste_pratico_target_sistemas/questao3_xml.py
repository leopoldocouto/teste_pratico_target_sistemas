...
class Faturamento:
    ...
    @staticmethod
    def carregar_dados_xml(caminho_arquivo: str) -> str:
    ...
    @staticmethod
    def converte_xml_json(dados_xml: str) -> str:
        root = ET.fromstring(f'<root>{dados_xml}</root>')
        data: type_json = []
        for row in root.findall('row'):
            dia: int = int(row.find('dia').text)
            valor: float = float(row.find('valor').text)
            data.append({'dia': dia, 'valor': valor})
        return json.dumps(data, indent=4)

    @staticmethod
    def menor_faturamento(dados_faturamento: type_json) -> float:
        faturamento_xml: type_json = dados_faturamento
        menor_valor: float = float('inf')
        for faturamento_dia in faturamento_xml:
